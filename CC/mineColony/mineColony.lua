...
wpp.wireless.connect("mineColony")

-- Initalize chatbox
local chatbox = wpp.peripheral.wrap("wpp@mineColony://44/chatBox_0")
...
local monitor = wpp.peripheral.wrap("wpp@mineColony://44/monitor_0")
...
local storage = "minecraft:chest_33"
            ...
            if provided < needed then
                if rs.isItemCrafting({item}) then
                    ...
                    if rs.craftItem({name=item, count=needed}) then
                        ...
                        color = colors.red
                        print("[Failed]", item)
                        chatbox.sendMessage("Need " .. needed .. " " .. " of " .. item)
                        os.sleep(1)
                    end
                end
            end
...
if equipFlag then
    chatbox.sendMessage("Check MineColony Equipment Requests")
    os.sleep(1)
    equipFlag = false
end
